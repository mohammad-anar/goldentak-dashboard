  ...
  return (
    <div className="flex flex-col gap-8 py-8 md:py-10 px-4 lg:px-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Current Login Users</h1>
          <p className="text-gray-500 mt-1">Monitor all device accounts, credentials, and subscriptions.</p>
        </div>
          ...
          <Table>
            <TableHeader className="bg-gray-50/40 border-b border-gray-100">
              <TableRow className="border-none">
                <TableHead className="font-bold text-[13px] text-gray-400 py-5 px-6 uppercase">Device & User Info</TableHead>
                <TableHead className="font-bold text-[13px] text-gray-400 py-5 px-6 uppercase text-center">Subscription Status</TableHead>
                <TableHead className="font-bold text-[13px] text-gray-400 py-5 px-6 uppercase text-center">Plan Tier</TableHead>
                <TableHead className="font-bold text-[13px] text-gray-400 py-5 px-6 uppercase">Start Date</TableHead>
                <TableHead className="font-bold text-[13px] text-gray-400 py-5 px-6 uppercase">Expiry / End Date</TableHead>
                  ...
                  return (
                    <TableRow key={user.id} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                      <TableCell className="py-4 px-6">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[11px] text-gray-400">
                            {user.name || user.email || user.username || "No profile details"}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="text-center py-4 px-6">
                        ...
                        `}>
                          {isSubscribed ? 'Subscribed' : 'Free / Expired'}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center py-4 px-6">
                        <Badge className={`
                      ...
                      <TableCell className="text-gray-500 py-4 px-6 text-xs font-medium">
                        {user.subscription?.startDate 
                          ? new Date(user.subscription.startDate).toLocaleDateString("en-US", { year: 'numeric', month: 'short', day: 'numeric' }) 
                          : "-"}
                      </TableCell>
                      <TableCell className="text-gray-500 py-4 px-6 text-xs font-medium">
